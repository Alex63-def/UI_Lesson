...
{
	CheckBoxStyle = FCoreStyle::Get().GetWidgetStyle<FCheckBoxStyle>("Checkbox"); //дефолтные значения
	TextStyle = FCoreStyle::Get().GetWidgetStyle<FTextBlockStyle>("NormalText"); 
	// FCoreStyle - структура в которой храняться все дефолтные значения всего движка
}
